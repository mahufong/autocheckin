    ...
    def push_telegram(self, title: str = None, content: str = None) -> None:
        if self.telegram_bot_token:
            try:
                ...
                log(
                    f"Telegram Response HTTP Status Code: {response.status_code}")
                log(
                    f"Telegram Response HTTP Response Body: {response.content}")
            except Exception as e:
        ...
        if self.wechat_qywx_am:
            try:
                ...
                if response["errmsg"] == 'ok':
                    ...
                    log("Failed to send wechat message. errmsg: {}".format(
                        response['errmsg']), level="error")
            except Exception as e:
        ...
        if self.dingtalk_access_token:
            try:
                ...
                if errcode == 0:
                    ...
                    log(
                        f"Failed to send Dingtalk message: {errmsg}", level="error")
            except Exception as e:
    ...
    async def _open_browser_async(self) -> None:
        if not self.browser_opened:
            self.browser = await launch(
                ...
                userDataDir=None if self.data_dir == None else os.path.abspath(
                    self.data_dir),
    ...
    if args.once:
        ...
        if not args.no_startup_notification:
            claimer_notifications.notify(
                local_texts.NOTIFICATION_TITLE_START, local_texts.NOTIFICATION_CONTENT_START)
        claimer.run_once(args.interactive, args.email, args.password,
                         args.verification_code, retries=args.debug_retries, raise_error=raise_error)
    else:
        if not args.no_startup_notification:
            claimer_notifications.notify(
                local_texts.NOTIFICATION_TITLE_START, local_texts.NOTIFICATION_CONTENT_START)
        claimer.run_once(args.interactive, args.email, args.password,
                         args.verification_code, retries=args.debug_retries)
        claimer.scheduled_run(args.run_at, args.interactive,
                              args.email, args.password, args.verification_code)
...
def main_handler(event: Dict[str, str] = None, context: Dict[str, str] = None) -> str:
    ...
    result_message = f"{local_texts.NOTIFICATION_CONTENT_CLAIM_SUCCEED}{claimed_item_titles}" if len(
        claimed_item_titles) else local_texts.NOTIFICATION_CONTENT_OWNED_ALL
